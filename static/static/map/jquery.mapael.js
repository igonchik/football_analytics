...
(function($) {
	 ...
	 */
	$.fn.mapael.drawLegend = function (legendOptions, $container, options, legendType, elems, scale, columns, legendIndex) {
		var $legend = {}
			, paper = {}
			, width = 0
			, height = 0
		...
		var ipercolumns = legendOptions.slices.length/columns;

		// Draw legend elements (circle, square or image in vertical or horizontal mode)
		for (var icol = 0, len = columns; icol<len; ++icol)
			for(i = icol*ipercolumns, length = ipercolumns*(icol+1); i < length; ++i) {
				...
				if(legendType == "area") {
					if (legendOptions.mode == "horizontal") {
						...
						//BY GONCH
						x = legendOptions.marginLeft+colwidth*icol;
						y = height - icol*30*ipercolumns;
					}
				...
				if (legendOptions.mode == "horizontal") {
					...
					//by gonch
					x = legendOptions.marginLeft + elemBBox.width + legendOptions.marginLeftLabel+colwidth*icol;
					y = height + (elemBBox.height / 2) - icol*30*ipercolumns;
				}
	...
	$.fn.mapael.createLegends = function ($container, options, legendType, elems, scale) {
		...
		if ($.isArray(options.legend[legendType])) {
			for (var j = 0; j < options.legend[legendType].length; ++j) {
				legends.push($.fn.mapael.drawLegend(options.legend[legendType][j], $container, options, legendType, elems, scale, cols, j));
			}
		} else {
			legends.push($.fn.mapael.drawLegend(options.legend[legendType], $container, options, legendType, elems, scale, cols));
		}
		return legends;
	};
