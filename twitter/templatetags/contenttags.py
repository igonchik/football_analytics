...
def astree(items, attribute):
    ...
    for item in items:
        if item.cp_id.path_id in items.values_list('cp_id_id', flat=True):
            parent_map[item.cp_id.path.tpathtr_set.all()[0]].append(item)
        else:
            parent_map[None].append(item)
...
class TreeNode(template.Node):
    ...
    def render(self, context):
        ...
        return render_items(self.tree.resolve(context), 0)
